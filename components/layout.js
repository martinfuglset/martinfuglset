...
export default function Layout({ children }) {
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-50">
      ...
      <header className="w-full max-w-5xl px-8 py-4">
      ...
      {/* Navigation (now part of the Layout) */}
      <nav className="w-full max-w-5xl px-8 py-4">
      ...
      <main className="flex flex-1 w-full px-8 py-4 max-w-5xl text-left">
        {children}
      </main>

      <footer className="w-full px-8 py-4 mt-8 max-w-5xl flex items-center justify-between">
        ...
        <p className="ml-6 text-gray-500">&copy; {new Date().getFullYear()} Martin Fuglset. All rights reserved.</p>
      </footer>
    </div>
  );
}
